...
- **Lv.1** — Base: Analyze staged changes, draft structured commit message, enforce human authorship, no-emoji rule, time tracking, sensitive file detection.
- **Lv.2** — Auto-Commit: Removed approval gate — AI analyzes, drafts, and commits in one seamless flow.
- **Lv.3** — Vigilant: Proactive post-task detection — auto-checks `git status` after completing any task and commits if dirty.
