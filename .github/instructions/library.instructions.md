...
- **Lv.1** — Base: Dynamic library scanning + keyword matching + deduplication prevention. Scans library/ at runtime, extracts keywords from topic, matches against filenames and section names, reads top matches to check overlap, reports findings.
...
- **Lv.3** — Commit Chain: After saving/updating library entries, auto-triggers the Auto-Commit skill (if installed) to commit all changes.
- **Lv.4** — Format-Aware Save: Auto-determines library section from content keywords, loads matching format template.
- **Lv.5** — Item Install: Install pre-made library entries from `library-items/` catalog.
